...
"""
This code is mostly taken from scikit-learn 21.03, as
scipy.optimize.linear_assignment is slightly different
------------------------------------------------------
...
def compute_metrics(labels: np.ndarray, predictions: np.ndarray):
    return {
        "accuracy": cluster_accuracy(true_row_labels=labels, predicted_row_labels=predictions),
        "nmi": normalized_mutual_info_score(labels_true=labels, labels_pred=predictions),
        "ari": adjusted_rand_score(labels_true=labels, labels_pred=predictions)
    }
...
def get_assignment(true_row_labels, predicted_row_labels):
...
class _HungarianState:
    ...
    def __init__(self, cost_matrix):
        ...
        self.Z0_r = 0
        self.Z0_c = 0
        self.path = np.zeros((n + m, 2), dtype=np.int_)
        self.marked = np.zeros((n, m), dtype=np.int_)
